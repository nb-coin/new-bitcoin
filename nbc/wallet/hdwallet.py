...
class HDWallet(object):
  ...
  def address(self, verByte=None):  # only support compressed address
    if verByte == None:
      verByte = b'\x00' if not self._testnet else b'\x6f'
    return util.key.publickey_to_address(point_compress(self.point()),self.vcn,verByte)
  ...
  def from_extended_key(extended_key):
    ...
    return HDWallet( key, chaincode, vcn=self.vcn,
  ...
  def load_from_cfg(cfg, passphrase=''):
    ...
    return HDWallet( prvKey or pubKey, unhexlify(cfg['chain']), vcn=cfg['vcn'],
      testnet=cfg['testnet'], depth=cfg['depth'],
